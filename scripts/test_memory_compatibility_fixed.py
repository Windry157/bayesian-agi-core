...
def test_interface_compatibility():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 验证返回类型和长�?
        assert len(current_ids) == len(optimized_ids) == 20, "添加数量不匹�?
        ...
        # 验证添加的项数量
        assert len(current_store.items) == len(optimized_store.items) == 20, "添加的项数量不匹�?
        print("  �?添加的项数量正确")
        ...
        print(f"  �?retrieve() 返回类型正确，结果数�? {len(optimized_results)}")
        ...
        if current_results and optimized_results:
            ...
            assert current_keys == optimized_keys, "返回字段不匹�?
        ...
        assert len(current_search) == len(optimized_search), "搜索结果数量不匹�?
        print(f"  �?search() 返回类型正确，结果数�? {len(optimized_search)}")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for query in queries:
            ...
            if len(current_result) != len(optimized_result):
                print(f"  �?{query}: 当前 {len(current_result)} 项，优化 {len(optimized_result)} �?)
                continue
            ...
            if current_contents != optimized_contents:
                print(f"  �?{query}: 结果内容不一�?)
                print(f"    当前: {current_contents}")
                print(f"    优化: {optimized_contents}")
            else:
                print(f"  �?{query}: 结果一�?({len(current_result)} �?")
        ...
        # 验证数据恢复
        assert len(optimized_store2.items) == len(test_data), "数据持久化失�?
        print(f"  �?数据持久化成功，恢复 {len(optimized_store2.items)} 条记�?)
        ...
        print(f"  �?批量添加成功，添�?{len(batch_ids)} 条记�?)
...
def test_error_handling():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert any(special_content in r["content"] for r in retrieved), "特殊字符检索失�?
            print(f"  �?特殊字符处理成功")
        except Exception as e:
        ...
        try:
            ...
            print(f"  �?超长内容处理成功，长�? {len(long_content)}")
        except Exception as e:
        ...
        try:
            ...
            print(f"  �?top_k=0 处理: 返回 {len(result)} �?)
        except Exception as e:
        ...
        try:
            ...
            print(f"  �?top_k=-1 处理: 返回 {len(result)} �?)
        except Exception as e:
...
def test_performance_comparison():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"  当前版本: {current_time*1000:.2f}ms ({num_items/current_time:.2f} �?�?")
        print(f"  优化版本: {optimized_time*1000:.2f}ms ({num_items/optimized_time:.2f} �?�?")

        if optimized_time < current_time:
            improvement = ((current_time - optimized_time) / current_time) * 100
            print(f"  �?优化版本更快，提�?{improvement:.1f}%")
        else:
            print(f"  �?优化版本变慢")
        ...
        if diff_percent < 20:
            print(f"  �?检索性能差异在可接受范围�?({diff_percent:.1f}%)")
        else:
