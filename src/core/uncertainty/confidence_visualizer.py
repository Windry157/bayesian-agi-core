...
class ConfidenceVisualizer:
    ...
    def visualize_confidence(
        ...
        try:
            ...
            message_info = self.CONFIDENCE_MESSAGES.get(
                confidence_level, self.CONFIDENCE_MESSAGES["unknown"]
            )
            ...
            visualization = {
                ...
                "display": {
                    "emoji": (
                        message_info["emoji"]
                        if self.display_settings["show_emoji"]
                        else ""
                    ),
                    "title": message_info["title"],
                    "description": message_info["description"],
                    "user_guidance": (
                        message_info["user_guidance"]
                        if self.display_settings["show_user_guidance"]
                        else None
                    ),
    ...
    def _format_uncertainty_sources(
        self, confidence_details: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """格式化不确定性来源
        ...
        for source in sources:
            ...
            source_info = self.UNCERTAINTY_SOURCE_MESSAGES.get(
                source_type,
                {"title": source_type, "recommendation": source.get("description", "")},
            )
    ...
    def generate_user_feedback_suggestions(
        self, confidence_level: str, uncertainty_sources: List[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """生成用户反馈建议
        ...
        if confidence_level == "high":
            suggestions["primary_action"] = {"text": "接受答案", "type": "accept"}
...
class ConfidenceFormatter:
    ...
    def format_badge(confidence_score: float) -> Dict[str, Any]:
        ...
        return {"label": label, "color": color, "score": confidence_score}
