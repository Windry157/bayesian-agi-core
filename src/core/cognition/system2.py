...
class System2:
    ...
    def make_decision(self, situation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 执行深度推理
        reasoning_chain = self._deep_reasoning(
            situation, best_action, self.reasoning_depth
        )
    ...
    def _deep_reasoning(
        self, situation: Dict[str, Any], action: str, depth: int
    ) -> List[Dict[str, Any]]:
        """深度推理
    ...
    def _simulate_action_result(
        self, situation: Dict[str, Any], action: str, step: int
    ) -> Dict[str, Any]:
        """模拟行动的结果
    ...
    def _evaluate_decision(
        ...
        average_confidence = (
            total_confidence / len(reasoning_chain) if reasoning_chain else 0.5
        )
